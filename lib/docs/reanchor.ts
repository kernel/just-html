...
/**
 * Re-anchor all anchored comments of a doc against the new content, inside the
 * caller's transaction. `edits` is provided for patch writes (enables tier 1);
 * pass undefined for full rewrites (tier 2 only). `newVersion` is stamped onto
 * anchored_version for every comment we successfully (re)resolve.
 *
 * Doc-level comments (anchor IS NULL) are skipped entirely — they have nothing
 * to anchor and never orphan.
 */
export async function reanchorComments(
  ...
  const { rows } = await client.query(
    `SELECT id, anchor, anchored_version, orphaned
       FROM comments
      WHERE doc_id = $1 AND deleted_at IS NULL AND anchor IS NOT NULL`,
    [docId]
  );
  ...
  for (const c of comments) {
    ...
    if (resolvedStart !== null && resolvedEnd !== null) {
      ...
      // Tier 3: orphan (not_found or ambiguous). Keep the comment + its anchor
      // selector so a later restoring edit can un-orphan it; just mark orphaned.
      if (!c.orphaned) {
        await client.query(
          `UPDATE comments SET orphaned = true, anchored_version = $2 WHERE id = $1`,
          [c.id, newVersion]
        );
      }
    }
