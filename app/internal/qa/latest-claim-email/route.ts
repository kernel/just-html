...
// QA ESCAPE HATCH — REMOVABLE POST-LAUNCH (the ONE claim flow).
//
// GET /internal/qa/latest-claim-email?email=… , guarded by header X-QA-Secret
// matching env QA_SECRET. Returns the most recent claim-email code for that
// email, stored in qa_claim_emails only when QA_SECRET is set (see
// app/agent/identity/route.ts and app/agent/identity/claim/route.ts). Reviewers
// depend on this to complete the claim flow programmatically (the code is
...
export async function GET(req: Request): Promise<Response> {
  ...
  const { rows } = await query<{ code: string; created_at: string }>(
    `SELECT code, created_at FROM qa_claim_emails
     WHERE email = $1
  ...
  return json({ email, code: rows[0].code, created_at: rows[0].created_at }, 200);
}
